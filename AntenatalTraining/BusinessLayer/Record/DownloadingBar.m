//
//  DownloadingBar.m
//  AntenatalTraining
//
//  Created by test on 16/5/28.
//  Copyright © 2016年 kimoworks. All rights reserved.
//

#import "DownloadingBar.h"

@implementation DownloadingBar

- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        
        UIVisualEffect *blur = [UIBlurEffect effectWithStyle:UIBlurEffectStyleExtraLight];
        UIVisualEffectView *effectView = [[UIVisualEffectView alloc] initWithEffect:blur];
        [self addSubview:effectView];
        [effectView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.edges.equalTo(self);
        }];
        
        self.backgroundColor = [UIColor colorWithWhite:1 alpha:kBarAlpha];
        
        TouchButton *pauseBtn = [TouchButton buttonWithType:UIButtonTypeCustom];
        [self addSubview:pauseBtn];
        [pauseBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.width.mas_equalTo(84);
            make.left.equalTo(self).offset(15);
            make.top.equalTo(self);
            make.bottom.equalTo(self);
        }];
        pauseBtn.tintColor = kThemeColor;
        [pauseBtn setTitleColor:kThemeLightBlack forState:UIControlStateNormal];
        
        pauseBtn.tag = 0;
        self.pauseBtn = pauseBtn;
        
        TouchButton *editBtn = [TouchButton buttonWithType:UIButtonTypeCustom];
        [self addSubview:editBtn];
        [editBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self);
            make.bottom.equalTo(self);
            make.right.equalTo(self).offset(-15);
            make.width.mas_equalTo(50);
        }];
        [editBtn setTitleColor:kThemeLightBlack forState:UIControlStateNormal];
        [editBtn setTitle:@"编辑" forState:UIControlStateNormal];
        editBtn.tag = 2;
        self.editBtn = editBtn;
        
        TouchButton *cancelBtn = [TouchButton buttonWithType:UIButtonTypeCustom];
        [self addSubview:cancelBtn];
        [cancelBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self);
            make.bottom.equalTo(self);
            make.left.equalTo(self).offset(15);
            make.width.mas_equalTo(50);
        }];
        [cancelBtn setTitleColor:kThemeLightBlack forState:UIControlStateNormal];
        [cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
        cancelBtn.tag = 3;
        self.cancelBtn = cancelBtn;
        
        self.pauseBtn.hidden = NO;
        self.editBtn.hidden = NO;
        self.cancelBtn.hidden = YES;
        self.pauseBtn.titleLabel.font = [UIFont systemFontOfSize:15];
        self.editBtn.titleLabel.font = [UIFont systemFontOfSize:15];
        self.cancelBtn.titleLabel.font = [UIFont systemFontOfSize:15];
        [self.pauseBtn addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
        [self.editBtn addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
        [self.cancelBtn addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
    }
    return self;
}

- (void)pause:(BOOL)pause {
    NSString *imageName = pause ? @"play_download" : @"pause_download";
    NSString *title = pause ? @"开始下载" : @"暂停下载";
    [self.pauseBtn setImage:[[UIImage imageNamed:imageName]imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate] forState:UIControlStateNormal];
    [self.pauseBtn setTitle:title forState:UIControlStateNormal];
    self.pauseBtn.tag = pause ? 0 : 1;
}

- (void)buttonAction:(TouchButton *)sender {
    
    NSInteger tag = sender.tag;
    switch (tag) {
        case 0:
        case 1:{
            [self pause:(tag == 1)];
            break;
        }
        case 2:{
            sender.tag = 3;
            [sender setTitle:@"完成" forState:UIControlStateNormal];
            
            self.pauseBtn.hidden = YES;
            self.editBtn.hidden = NO;
            self.cancelBtn.hidden = NO;
            break;
        }
        case 3:{
            self.editBtn.tag = 2;
            [self.editBtn setTitle:@"编辑" forState:UIControlStateNormal];
            
            self.pauseBtn.hidden = NO;
            self.editBtn.hidden = NO;
            self.cancelBtn.hidden = YES;
            break;
        }
        case 4:{
            self.editBtn.tag = 2;
            [self.editBtn setTitle:@"编辑" forState:UIControlStateNormal];
            
            self.pauseBtn.hidden = NO;
            self.editBtn.hidden = NO;
            self.cancelBtn.hidden = YES;
            break;
        }
        default:
            break;
    }
    
    if (_delegate && [_delegate respondsToSelector:@selector(downloadingBar:didTouchedAtIndex:)]) {
        [_delegate downloadingBar:self didTouchedAtIndex:tag];
    }
}

@end
